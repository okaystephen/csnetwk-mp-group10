...
/** This is our GUI Controller class
 * @author SALAMANTE, Stephen E.
 * @author CARANDANG, Pauline
*/

public class ControllerTest {
	private CustomerInterface m_model;
    private MenuTest m_menu;
    private Gui m_gui;
	...
	ControllerTest(CustomerInterface model,Gui guiview){
		
		m_gui = guiview;
		m_model = model;

		guiview.addyesBtnListener(new YesBtnListener());
		guiview.answerListener(new AnswerListener());
		guiview.addcloseListener(new CloseBtnListener());
	}
	...
	class SubmitLoadListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				+ "<br>Tracking Number: " + loadString[3]
				+ "<br>Recipient Name: " + loadString[2]
				+ "<br>Date ordered: " + loadString[0]
				+ "<br>Current date: " + sCurrentDate
				+ "<br>Date of arrival: " + loadString[1]
				+ "<br>Status: " + sTrackingStatus + "</html>");
				scanstatus.close();
	...
	class ProceedLoadListener implements ActionListener {
	 ...
	 /** Listener for the submit button that would check if user chooses document or product */
	class SubmitListener implements ActionListener {
	 ...
	 /** Listener for the submit button that the action 
	  *  is equivalent to scanItem method in CustomerInterface
	  */
	class SubmitListener1 implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			while (iChecker == 0){
				...
				if(m_model.getRecipient().getRegion() == 1 || m_model.getRecipient().getRegion() == 2 || m_model.getRecipient().getRegion() == 3 || m_model.getRecipient().getRegion() == 4){
					iChecker++;
				}
			...
			if(m_model.getRecipient().getItems() <= 0) {
				...
				if(m_model.getItem() instanceof Document == true){
					...
					if (m_model.getItem().getnoPage() < 0 && m_model.getItem().getdWidth() < 0 && m_model.getItem().getdLength() < 0) {
						m_menu.showMessage("Wrong input, try again.");
					}
				...
				// For product type of item
					...
					if(m_model.getItem().getProductType() == true){
						...
						m_model.getItem().setdWeight(weight);	// Customer inputs weight of regular product
							if(m_model.getItem().getdWidth() <0 && m_model.getItem().getdLength() <0 && m_model.getItem().getdWeight() <0 && m_model.getItem().getdHeight() <0) {
								m_menu.showMessage("Wrong input, try again.\n");
							}
		...
		if (Checker == m_model.getRecipient().getItems()) {
			...
			for (int i = 0; i < m_model.getCustomerItem().size(); i++){
				if (m_model.getCustomerItem().get(i) instanceof Document){
					itemNames[i] = ((i+1) +". Document: " + m_model.getCustomerItem().get(i).getdLength() + " x " + m_model.getCustomerItem().get(i).getdWidth());
				} else{
					itemNames[i] = ((i+1) +". Product: " + m_model.getCustomerItem().get(i).getdLength() + " x " + m_model.getCustomerItem().get(i).getdWidth() + " x " + m_model.getCustomerItem().get(i).getdHeight());
				}
			}
	...
	class ChoiceParcelListener1 implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			int getParcel;
				...
				m_menu.HideParcelPanel();
				if(iLoop>0){
				m_model.getParcel().get(m_model.getParcelNo()).getTracker().TrackingNumber(m_model.getRecipient(), m_model.getShippingSequence());
				m_menu.setbreakD("<html><br>[Fee Breakdown]"
						+ "<br>Destination (" + m_model.getDestination() + "): PHP " + m_model.computeDestFee()
						+ "<br>Parcel and Weight: PHP " + m_model.computeFee()
						+ "<br>Insurance: PHP " + m_model.computeInsurace()
						+ "<br>Total: PHP " + (m_model.computeDestFee() + m_model.computeFee() + m_model.computeInsurace())
						+ "<br>Tracking Number: " + (m_model.getParcel().get(m_model.getParcelNo()).getTracker().getTrackingNumber())
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (int i = 0; i < m_model.getParcel().size(); i++){
				if (trackNum.equals(m_model.getParcel().get(i).getTracker().getTrackingNumber())){
					...
					m_menu.setTrackInfo("<html>[Tracking Number: " + trackNum + "]"
					+ "<br>Recipient Name: " + m_model.getParcel().get(i).getTracker().getDateEssentials(2)
					+ "<br>Date Ordered: " + m_model.getParcel().get(i).getTracker().getDateEssentials(0)
					+ "<br>Current Date: " + m_model.getParcel().get(i).getTracker().getCurrentDate()
					+ "<br>Date of Arrival: " + m_model.getParcel().get(i).getTracker().getDateEssentials(1)
					+ "<br>Shipping Status: " + m_model.getParcel().get(i).getTracker().getShippingStat()
					);
